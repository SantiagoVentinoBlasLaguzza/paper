...
# MODIFICATION: Import ReduceLROnPlateau
...
PT_FILES_DIR = "matrix"
OUTPUT_MODEL_DIR = "cvae_pytorch_models_AD_CN_classifier_phd_v4" # MODIFICATION: Changed version for new run
...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                        logging.FileHandler(os.path.join(OUTPUT_MODEL_DIR, "training_log_phd_v4.txt")), # MODIFICATION: Changed version
...
CLASSIFIER_DENSE_UNITS = [32]       # MODIFICATION: Was [64]
CLASSIFIER_DROPOUT_RATE = 0.2     # MODIFICATION: Was 0.5
CLASSIFIER_EPOCHS = 200           # MODIFICATION: Was 150
CLASSIFIER_LR = 1e-4              # MODIFICATION: Was 3e-4
CLASSIFIER_WEIGHT_DECAY = 1e-4    # MODIFICATION: Was 1e-3
CLASSIFIER_BATCH_SIZE = 16        # NEW: Specific batch size for classifier
NUM_BINARY_CLASSES = 2

EARLY_STOPPING_PATIENCE_CVAE = 35
EARLY_STOPPING_PATIENCE_CLF = 50  # MODIFICATION: Was 25
...
class EarlyStopping:
    ...
    def __call__(self, current_val_metric, model):
        ...
        elif score < self.best_score + self.delta: # For min mode, if score (neg metric) is less than best_score (neg best_metric) + delta
            self.counter += 1
...
class IdentityBlock(nn.Module):
    def __init__(self, in_channels, filters, kernel_size, num_groups_for_gn): # MODIFICATION: Added num_groups_for_gn
        super(IdentityBlock, self).__init__()
        filters1, filters2, filters3 = filters
        self.conv1 = nn.Conv2d(in_channels, filters1, kernel_size=1, stride=1, padding=0, bias=False)
        # MODIFICATION: BatchNorm2d to GroupNorm
        self.gn1 = nn.GroupNorm(num_groups_for_gn if filters1 % num_groups_for_gn == 0 else filters1 // (filters1//num_groups_for_gn) if (filters1//num_groups_for_gn)>0 else 1 , filters1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(filters1, filters2, kernel_size=kernel_size, stride=1, padding=(kernel_size-1)//2, bias=False)
        # MODIFICATION: BatchNorm2d to GroupNorm
        self.gn2 = nn.GroupNorm(num_groups_for_gn if filters2 % num_groups_for_gn == 0 else filters2 // (filters2//num_groups_for_gn) if (filters2//num_groups_for_gn)>0 else 1, filters2)
        self.conv3 = nn.Conv2d(filters2, filters3, kernel_size=1, stride=1, padding=0, bias=False)
        # MODIFICATION: BatchNorm2d to GroupNorm
        self.gn3 = nn.GroupNorm(num_groups_for_gn if filters3 % num_groups_for_gn == 0 else filters3 // (filters3//num_groups_for_gn) if (filters3//num_groups_for_gn)>0 else 1, filters3)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, filters, kernel_size, stride=2, num_groups_for_gn=32): # MODIFICATION: Added num_groups_for_gn
        ...
        # MODIFICATION: BatchNorm2d to GroupNorm
        self.shortcut_gn = nn.GroupNorm(num_groups_for_gn if filters3 % num_groups_for_gn == 0 else filters3 // (filters3//num_groups_for_gn) if (filters3//num_groups_for_gn)>0 else 1, filters3)
...
class CVAE_ResNetEncoder(nn.Module):
    def __init__(self, img_channels, img_res, condition_dim, latent_dim,
                 ...
                 num_groups_for_gn): # MODIFICATION: Added num_groups_for_gn
        ...
        self.num_groups_for_gn = num_groups_for_gn # Store for internal use if needed, or pass directly

        encoder_modules = []
        current_channels = img_channels
        current_size = img_res
        
        # Helper for GroupNorm creation
        def _get_gn(num_channels_gn):
            # Ensure num_groups is appropriate for num_channels
            actual_num_groups = self.num_groups_for_gn
            if num_channels_gn < self.num_groups_for_gn or num_channels_gn % self.num_groups_for_gn != 0:
                # Find largest power of 2 less than or equal to num_channels_gn and also <= self.num_groups_for_gn
                # Or simply use a smaller number of groups, e.g., try to make groups of size 2, 4, 8...
                # For simplicity, if num_groups_for_gn doesn't divide num_channels_gn,
                # find the largest factor of num_channels_gn that is <= num_groups_for_gn
                # This is a bit complex, let's simplify: find a good common divisor or use 1 group if too small
                if num_channels_gn < self.num_groups_for_gn : # if channels less than groups, make groups = channels (instance norm like)
                     actual_num_groups = num_channels_gn if num_channels_gn > 0 else 1 # handle 0 channels if it occurs
                elif num_channels_gn % self.num_groups_for_gn != 0: # if not divisible
                    # Fallback: try to find a smaller number of groups like 16, 8, 4, 2, 1 that divides channels
                    common_divs = [g for g in [16,8,4,2,1] if num_channels_gn % g == 0 and g <= self.num_groups_for_gn]
                    actual_num_groups = common_divs[0] if common_divs else 1 # Default to 1 group (LayerNorm like)
            return nn.GroupNorm(actual_num_groups, num_channels_gn)
...
class LatentSpaceBinaryClassifier(nn.Module):
    def __init__(self, latent_dim, num_classes, hidden_units_list, dropout_rate=0.5): # MODIFICATION: dropout_rate default changed in hyperparams
        ...
        for hidden_units in hidden_units_list:
            layers_list.extend([
                ...
                nn.BatchNorm1d(hidden_units), # Using BatchNorm1d for MLP, could also be LayerNorm
...
def evaluate_binary_classifier(classifier, dataloader, criterion, device):
    ...
    auc_score = roc_auc_score(all_labels_list, all_probs_positive_class) if len(np.unique(all_labels_list)) > 1 and total_samples > 0 else 0.5 # Handle case with only one class
...
if __name__ == "__main__":
    ...
    cvae_model = CVAE_ResNetEncoder(
        ...
        num_groups_for_gn=GN_NUM_GROUPS # MODIFICATION: Pass GN_NUM_GROUPS
    ...
    # ... (CVAE training loop remains the same)
    ...
    if cvae_val_losses_log and len(cvae_val_losses_log) > 0: plt.axvline(np.argmin(cvae_val_losses_log), color='r', linestyle='--', label=f'Best Val Epoch: {np.argmin(cvae_val_losses_log)+1}')
    ...
    # ... (extract_latent_and_filter function remains the same)
    cvae_model.eval() 
    ...
    # MODIFICATION: Use CLASSIFIER_BATCH_SIZE
    train_loader_clf = DataLoader(train_dataset_clf_final, batch_size=CLASSIFIER_BATCH_SIZE, shuffle=True)
    val_loader_clf = DataLoader(val_dataset_clf_final, batch_size=CLASSIFIER_BATCH_SIZE, shuffle=False)
    test_loader_clf_final = DataLoader(test_dataset_clf_final_eval, batch_size=CLASSIFIER_BATCH_SIZE, shuffle=False)

    binary_classifier = LatentSpaceBinaryClassifier(
        latent_dim=LATENT_DIM, num_classes=NUM_BINARY_CLASSES,
        hidden_units_list=CLASSIFIER_DENSE_UNITS, # MODIFIED via hyperparameter
        dropout_rate=CLASSIFIER_DROPOUT_RATE     # MODIFIED via hyperparameter
    ).to(DEVICE)
    ...
    optimizer_binary_clf = optim.Adam(binary_classifier.parameters(), lr=CLASSIFIER_LR, weight_decay=CLASSIFIER_WEIGHT_DECAY) # MODIFIED via hyperparameter
    # MODIFICATION: Changed scheduler to ReduceLROnPlateau
    scheduler_clf = ReduceLROnPlateau(optimizer_binary_clf, mode='max', factor=0.1, patience=10, verbose=True) # Monitor val_auc

    classifier_early_stopper = EarlyStopping(patience=EARLY_STOPPING_PATIENCE_CLF, verbose=True, # MODIFIED via hyperparameter
    ...
    for epoch in range(1, CLASSIFIER_EPOCHS + 1): # MODIFIED via hyperparameter
        ...
        scheduler_clf.step(val_auc_clf) # MODIFICATION: Pass val_auc_clf to scheduler
    ...
    # ... (Plotting and final evaluation remain the same)
    ...
    if clf_val_accs_log and len(clf_val_accs_log)>0: plt.axvline(np.argmax(clf_val_accs_log), color='r', linestyle='--', label=f'Best Val Epoch (Acc): {np.argmax(clf_val_accs_log)+1 }')
    ...
    if clf_val_losses_log and len(clf_val_losses_log)>0: plt.axvline(np.argmin(clf_val_losses_log), color='r', linestyle='--', label=f'Best Val Epoch (Loss): {np.argmin(clf_val_losses_log)+1}')
    ...
    if clf_val_auc_log and len(clf_val_auc_log)>0: plt.axvline(np.argmax(clf_val_auc_log) , color='r', linestyle='--', label=f'Best Val Epoch (AUC): {np.argmax(clf_val_auc_log)+1}')
    ...
    from sklearn.linear_model import LogisticRegression # Keep this import local if only used here
    try:
